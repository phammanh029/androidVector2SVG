	...
	for vdPath in vdPaths:
		...
		if vdPath.parentNode == vdContainer:
			...
			if vdPath.hasAttribute('android:fillColor'):
				color = vdPath.attributes['android:fillColor'].value
				if len(color) == 9:
					alpha = color[1:3]
					svgPath.attributes['fill'] = color.replace(alpha, '')
					svgPath.attributes['fill-opacity'] = str(round(int(alpha,16)/255.0, 2))
				else:
					svgPath.attributes['fill'] = color
			else:
